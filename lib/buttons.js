    ...
    if (status == 5) {
      ...
      conn.addHandler(function(msg){ 
        ...
        if (type == "chat" || type == "groupchat"  && elems.length > 0) {
          console.log("HIS IS ELEMNS 0" + Strophe.getText(elems[0]));
          console.log("HIS IS ELEMNS 1" + Strophe.getText(elems[1]));
...
//this is kind of messy and untested
              ...
              switch(action){
               ...
               case "display":
                var item = conn.bl.blink.items[name];
                item.nowp=j;
                // conn.bl.me.manifest = j.manisfest;     
                $(document).trigger('tv_changed',[item]);
